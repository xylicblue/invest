...
export default function PlayerDashboard({ profile, onLogout }) {
  ...
  const loadGames = async () => {
    try {
      ...
      // Filter to show only live and completed games for players
      setGames(data?.filter(g => g.status === 'live' || g.status === 'completed') || [])
    } catch (err) {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <div className={`inline-flex items-center justify-center rounded-full px-3 py-1 text-xs font-medium ring-1 ring-inset ${statusStyles[status] || statusStyles.live}`}>
        ...
        <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
            ...
            <button
              ...
              disabled={loading}
              className="flex min-w-[84px] cursor-pointer items-center justify-center gap-2 overflow-hidden rounded-lg h-10 px-4 bg-white/10 text-white text-sm font-bold leading-normal tracking-[0.015em] border border-white/20 hover:bg-white/20 transition-colors disabled:opacity-50"
            >
                ...
                >
                  ...
                  <div className="flex flex-col gap-2 text-sm">
                    <div className="flex justify-between text-white/80">
                      <span className="text-white/60">Starting Cash:</span>
